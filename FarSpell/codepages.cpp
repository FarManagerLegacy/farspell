...
/*
  by Рома Мик
  http://rsdn.ru/Forum/Message.aspx?mid=485281&only=1
  2005.12.22 (ss): some names from http://www.dotnet247.com/247reference/System/Text/Encoding/__rotor
*/
    { "437", 437 },
    { "_iso-2022-jp", 50221 },
...
const int langs_count = sizeof(langs)/sizeof(langs[0]);

int lang_t_fcmp(const void * _key, const void * _elt)
{
  const char* key = (const char*)_key;
  const lang_t *elt = (const lang_t *)_elt;
  return _stricmp(key, elt->name);
}

#ifdef _DEBUG
int lang_t_fcmp_qsort(const void * _elt1, const void * _elt2)
{
  const lang_t *elt1 = (const lang_t *)_elt1;
  const lang_t *elt2 = (const lang_t *)_elt2;
...
int search_codepage(FarString &name)
{
  /*for(unsigned i=0; i<sizeof(langs)/sizeof(langs[0]); i++)
    if (name.CompareNoCase(langs[i].name)==0)
      return langs[i].codepage_id;
  return -1;
  */
# ifdef _DEBUG
  static bool well_sorted = false;
  if (!well_sorted)
  {
    lang_t well_sorted_langs[langs_count];
    memcpy(well_sorted_langs, langs, sizeof(langs));
    FarSF::qsort(well_sorted_langs, langs_count, sizeof(lang_t), lang_t_fcmp_qsort);
    well_sorted = memcmp(well_sorted_langs, langs, sizeof(langs)) == 0;
    if (!well_sorted)
    {
      fprintf(stderr, "langs[] is unordered! See %s...\r\n", __FILE__);
      for (unsigned i=0; i< langs_count; i++)
        if (langs[i].name != well_sorted_langs[i].name)
          fprintf(stderr, "%d\t%s\t->\t%s\r\n", i, langs[i].name, well_sorted_langs[i].name);
      Sleep(10000);
      ExitThread(0);
    }
  }
# endif _DEBUG
  lang_t* found = (lang_t*)FarSF::bsearch(name.c_str(), langs, langs_count, sizeof(lang_t), lang_t_fcmp); 
  return found ? found->codepage_id : -1;
}